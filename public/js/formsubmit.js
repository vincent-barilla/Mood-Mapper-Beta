...
function formSubmit(){
  ...
  // Set "pauseBtn" to "Pause" to stay consistent with user's activity.      
  ...
  function serializeForm(){
    ...
    // Get the final tweet's from the last response. May be empty string. 
    ...
    // "mode" tells the server where to send the query parameters. Best kept separate from the query itself.
  ...
  function sendData(data){
    ...
    xhr.onreadystatechange = function (){
      ...
      if ((this.readyState === XMLHttpRequest.LOADING || this.readyState === XMLHttpRequest.DONE) && this.status === 200) {
        ...
        // '{"text":"'' delimits the start of a new tweet, according to formatting in "this.analyze" in "tweetAnalyzer.js"
