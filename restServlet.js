...
this.query = function(data, response, request, wordBank){ 
	...
	// Here's a sample query string: '/1.1/search/tweets.json?q=DonaldTrump%20since%3A2016-03-25%20until%3A2016-03-28&count=3',
	// followed by customizing the query to user parameters.
	...
	var req = new https.request(options, function(res){
		...
		// 'end' indicates all the data from the query is done sending. responseString can now be parsed into an object which contains 
		// 'statuses', which contains an array of the individual tweets as JSON objects. Loop through that array, pass each tweet to 
		// tweetAnalyzer, then write the result to the front end. Use a timeout to throttle the writing, so the geocoders don't fail.
		res.on('end', function(){ 
			var tweets = JSON.parse(responseString).statuses; 
			for (var i = 0; i < tweets.length; i++){ 
				(function intervalClosure(i){
					setTimeout(function(){    
						var result = TweetAnalyzer.analyze(tweets[i], wordBank); 
						if (result){
