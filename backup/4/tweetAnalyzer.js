				...
				*/
				...
				return stats;
 				...
 				function normalizeWord(word){
 					...
 					// Checks for null returns on matching capitalized letters, increases multiplier every time an all-cap word is found.
				...
				function scoreWord(word){
					...
					if (wordBank[word] > 0){
						stats['posWords'].push({'word': word,
												'score': wordBank[word],
												'EF' : multiplier});

						mood[0] -= wordBank[word] * 25 * multiplier;
						mood[1] += wordBank[word] * 25 * multiplier;												  
						mood[2] += wordBank[word] * 25 * multiplier;
					}
					if (wordBank[word] < 0){
						stats['negWords'].push({'word': word,
												'score': wordBank[word],
												'EF': multiplier});
						mood[0] += wordBank[word] * -25 * multiplier;
						mood[1] += wordBank[word] * 25 * multiplier;
				...
				// At least the Chrome browser recast RGB values to fit in the 0-255 uint8 range, but I do it here as a fail-safe. 
				function setMood(){
