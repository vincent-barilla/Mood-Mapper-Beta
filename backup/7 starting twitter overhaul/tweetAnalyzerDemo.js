			...
			function analyzeMood(){
				...
				function scoreIndividualWords(){
 				...
 				function normalizeWord(word){
 					...
 					// Flexible scoring template. At minimum needs gainers and constant. If no losers, just score gainers. Inner function abstracts the 
 					...
 					function scoringTemplate(gainers, constant, losers){
 						...
 						function score(object){
 							object.length > 1 && (constant.length == object.length) ? object.forEach(function(obj, i){obj += constant[i]}) : obj += constant;
 						}
 					...
 					function scoreCasing(word){
 						...
 						// Execute the scoring template methodology if the word is all caps.
 						if (caps && caps.length == word.length){
 							scoringTemplate([wordMultiplier, tweetMultiplier], [(.05 * caps.length), (.02 * caps.length)]
 						}
 					}
