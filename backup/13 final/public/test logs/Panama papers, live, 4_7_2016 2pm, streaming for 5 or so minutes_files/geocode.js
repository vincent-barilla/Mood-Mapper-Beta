    ...
    geoXHR.onreadystatechange = function(){
      if (geoXHR.readyState == XMLHttpRequest.DONE && geoXHR.status == 200) {
        ...
        if(response.results[0]){
          ...
          // 1 corresponds to mapQuest, the other case corresponds to Open Cage. The returns 
          // from the queries do not come in the same format, which is why it's necessary to have 
          // separate assignments according to the condition in the ternary operator -- i.e.,
          // the first option is how mapQuest results must be assigned, the second option is how 
          // Open Cage results must be assigned.
